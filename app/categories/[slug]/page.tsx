...
import { generateCategoryMetadata, generateStructuredData } from '@/lib/seo'
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const collectionStructuredData = generateStructuredData({
    type: 'CollectionPage',
    props: {
      name: `${category.metadata?.name || category.title} - Food Category`,
      ...
      mainEntity: {
        ...
        itemListElement: posts.slice(0, 10).map((post: Post, index: number) => ({
          '@type': 'Article',
          position: index + 1,
          name: post.title,
    ...
    props: {
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: 'Categories',
          item: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://foodjourney.com'}/categories`,
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: category.metadata?.name || category.title,
          item: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://foodjourney.com'}/categories/${category.slug}`,
        },
      ],
    },
  })
  ...
  return (
    <>
      <SEOHead structuredData={collectionStructuredData} />
      <script
