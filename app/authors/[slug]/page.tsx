...
import { generateAuthorMetadata, generateStructuredData } from '@/lib/seo'
...
export default async function AuthorPage({ params }: AuthorPageProps) {
  ...
  const personStructuredData = generateStructuredData({
    ...
    props: {
      ...
      knowsAbout: [
        ...
        'Travel Writing',
        'Culinary Tourism',
        author.metadata?.specialty || 'Food & Travel',
      ].filter(Boolean),
      sameAs: [
        author.metadata?.instagram ? `https://instagram.com/${author.metadata.instagram}` : '',
        author.metadata?.twitter ? `https://twitter.com/${author.metadata.twitter}` : '',
      ].filter(Boolean),
      worksFor: {
        '@type': 'Organization',
        name: 'FoodJourney',
    ...
    props: {
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: 'Authors',
          item: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://foodjourney.com'}/authors`,
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: author.metadata?.full_name || author.title,
          item: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://foodjourney.com'}/authors/${author.slug}`,
        },
      ],
    },
  })
  ...
  return (
    <>
      <SEOHead structuredData={personStructuredData} />
      <script
